...
<!-- It looks like you call a `mcSummary_function.R` file. -->
<!-- If you were distributing your template for someone else to run, you would need to include it with this document. -->
<!-- So, for example, I had to create a dummy function with that name in order to render your report. -->
...
<!-- A newer and more explicit way of calling functions with dependencies on external packages is this: `tables::tabular` -->

<!-- I suggest using a more descriptive name than `table`. -->
...
I don't know the `tables` package well.
It might not work especially well with `knitr::kable()`.

```{r}
class(table)
```

I suggest exploring the `kableExtra` [package](http://haozhu233.github.io/kableExtra/awesome_table_in_html.html).
...
# Again, I recommend using descriptive names.

# I commented-out the next command because I don't have code for this function.
#mcSummary(asscn)
